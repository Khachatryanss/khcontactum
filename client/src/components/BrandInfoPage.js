...
/**
 * Props:
 *  - info: public info object (brandInfos / brandWorkers ներսում)
...
export default function BrandInfoPage({ info, lang = "am", activeKeyword }) {
  ...
  // ընտրում ենք worker-ը keyword-ով, եթե չկա՝ առաջինը
  const selected =
  ...
  // գույներ՝ admin-ից
  const nameColor = selected.nameColor || "#000000";
  const bioColor = selected.bioColor || "#ffffff";
  const bioBgColor = selected.bioBgColor || "rgba(0,0,0,0.70)";
  ...
  return h(
    ...
    // bio block (տեքստի ու ֆոնի գույնները admin-ից)
    workerBio &&
      h(
        "div",
        {
