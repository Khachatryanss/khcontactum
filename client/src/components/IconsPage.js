...
export default function IconsPage({
  ...
  const moveDrag = (e) => {
    ...
    setOrbitAngle((a) => {
      let next = a + dx * 0.6;          // սենսիթիվություն
      next = ((next % 360) + 360) % 360; // միշտ [0, 360)
      return next;
    });
  };
  ...
  /* ===== ONE-COLUMN MODE – BrandsPage-ի նման ===== */
  ...
  return h(
    ...
    ...links.map((l = {}, i) => {
      ...
      if (layoutStyle === "dzev4") {
        const base = (360 / total) * i;          // հավասար բաժանում
        const angle = orbitAngle + base;        // ընդհանուր անկյուն
        const norm = ((angle % 360) + 360) % 360;

        const target = 270; // վերևի դիրքը (0° աջ, 90° ներքև, 180° ձախ, 270° վերև)
        const diffRaw = Math.abs(norm - target);
        const delta = Math.min(diffRaw, 360 - diffRaw);

        const isActive = delta <= 12; // մոտ 24° «slot»

        aProps.style = {
          "--angle": `${angle}deg`,
        };
        if (isActive) {
          aProps["data-active"] = "1";
        }
      }
